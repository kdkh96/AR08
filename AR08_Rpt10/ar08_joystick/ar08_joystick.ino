...
void loop(){
  ...
  int yValue = analogRead(yAxixPin);
  ...
  if(yValue > 540)
  {
    analogWrite(rStop,HIGH);
  }
  else{
    analogWrite(rStop,LOW);
  }

  if(xValue < 512)
  {
    analogWrite(yLeft,HIGH);
  }
  else{
    analogWrite(yLeft,LOW);
  }
  
  if(xValue > 540)
  {
    analogWrite(yRight,HIGH);
  }
  else{
    analogWrite(yRight,LOW);
  }
  ...
  if(zValue == LOW){
    ...
    delay(300);
    lcd.backlight();  
    analogWrite(wShoot,HIGH);  
  }
  else{
    analogWrite(wShoot,LOW);
  }
  delay(100);
}
